                ...
                <tbody class="text-gray-600 text-sm font-light">
                    @foreach ($testimonials as $testimonial)
                        <tr class="border-b border-gray-200 hover:bg-gray-100">
                            ...
                            <td class="py-3 px-6 text-left">{{ $testimonial->company }}</td>
                            <td class="py-3 px-6 text-center">{{ $testimonial->rating }} / 5</td>
                            <td class="py-3 px-6 text-center">
                                <span
                                    class="bg-{{ $testimonial->is_featured ? 'green' : 'red' }}-500 text-white py-1 px-3 rounded-full text-xs">
                                    {{ $testimonial->is_featured ? 'Yes' : 'No' }}
                                </span>

                                <a href="{{ route('admin.testimonials.toggle', $testimonial) }}">
                                    <span
                                        class="bg-{{ $testimonial->is_featured ? 'red' : 'green' }}-500 text-white py-1 px-3 rounded-full text-xs">
                                        {{ $testimonial->is_featured ? 'deactivate' : 'activate' }}
                                    </span>
                                </a>
                            </td>
                            <td class="py-3 px-6 text-center">
